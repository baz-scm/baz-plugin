...
`SAFE_REPO_ARG` is deliberately looser than `SAFE_REPO`: it makes the `/owner` half optional, because `baz-codebase-exploration` documents the **short leaf name** (`baz`) as a valid `repository` argument alongside the full `owner/repo`. Requiring the slash silently dropped short-form repos, so a session that searched one uploaded a plan that was not discoverable under it. `SAFE_REPO` stays strict for `repoFromCwd()`, which parses a git remote and always has both halves. **The character class is the security control, not the shape** — it is what keeps a newline, backtick, quote or space out of instruction text. One test asserts both directions at once: `baz` survives, and a name with spaces, backticks, `$(...)`, or more than one slash does not.
