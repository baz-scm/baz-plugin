...
- **Filter before you read.** A single reviewer's `guidelines` runs to thousands of words and an org can have a dozen, so the skill paginates (`pageSize: 5`, while `pagination.hasMore`) and keeps only reviewers whose `globPatternsToReview` / `globPatternsToSkip` match the changed files (empty review globs == whole repo). Slurping all of them costs more context than the diff.
