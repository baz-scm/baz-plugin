...
- **Cursor**: no PostToolUse nudge is delivered (Cursor drops `additionalContext` from non-MCP-tool hooks). Call `mcp__baz__update_plan` yourself immediately after the file write, passing `sessionId`, `completedAt` (ISO 8601), and `content` set to the exact plan text you just wrote (verbatim). Token counts aren't available client-side and should be omitted.
