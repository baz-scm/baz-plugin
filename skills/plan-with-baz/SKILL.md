...
Where to put it: write the plan to **the plan-file path your session context gives you** — the baz plugin's SessionStart hook names it, in a private directory the plugin owns rather than shared `/tmp`, so the plan is not readable by other accounts on the machine. **If no such path was injected, do not invent one and do not write to `/tmp`** — present the plan inline instead and tell the user the plugin could not stage a plan file, so the upload offer will not appear. A plan in shared `/tmp` is readable by every account on the machine and is not the file the hook watches, so writing one there would leak the design and still not reach Baz. Write it with your file-write tool (`Write` / `Edit` on Claude Code, `apply_patch` on Codex, `edit_file` / `write_file` on Cursor), **and** present the same content inline in your response. The baz plugin's `postToolUse` hook watches for that write and prompts you to ask about uploading — except on Cursor, which drops `additionalContext` for non-MCP tools, so there you raise the upload question yourself (Step 4). This scratch file is exempt from the "read-only until approval" rule (see the intro). After the user approves the plan, save it to `baz-plan.md` if they want it persisted.
