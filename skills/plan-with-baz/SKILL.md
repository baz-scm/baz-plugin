...
- **If they say yes**: call `mcp__baz__update_plan` with `sessionId` and `content` set to the exact plan text you wrote (verbatim, no summary). Where a hook supplied authoritative `content` / `tokensUsed` / `modelId` / `repoNames` values, pass those exactly as given rather than your own recollection or estimates. The tool result contains a **shareable plan link** — include it in your reply so the user can open the plan.
